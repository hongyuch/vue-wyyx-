...
import { reqMsite, reqShopGuide } from "../../api";
...
export default {
  ...
  async mounted() {
    // 发送请求，获取数据
    const result1 = await reqMsite();
    if (result1.code === 0) {
      this.kingkongs = result1.data;
    }

    const result2 = await reqShopGuide();
    console.log(result2);
    if (result2.code === 0) {
      this.shopGuides = result2.data;
    }
    // 创建一个swiper对象
